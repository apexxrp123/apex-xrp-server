...
const server = http.createServer(async (req, res) => {
  ...
  // --- APEX airdrop whitelist (Testnet): players who reach rank level 3+ ---
  if (!globalThis.__apexAirdropWl) globalThis.__apexAirdropWl = [];
  if (req.method === "POST" && url.pathname === "/airdrop/whitelist") {
    try {
      ...
      const list = globalThis.__apexAirdropWl;
      const existing = list.find((e) => e.address === address);
      if (existing) {
        sendJson(res, 200, { ok: true, replay: true, entry: existing, count: list.length });
        return;
      }
      const entry = {
        address,
      ...
      sendJson(res, 200, { ok: true, entry, count: list.length });
    } catch (e) {
  ...
  if (req.method === "GET" && url.pathname === "/airdrop/whitelist") {
    const list = globalThis.__apexAirdropWl || [];
    sendJson(res, 200, { ok: true, count: list.length, entries: list.slice(-500) });
    return;
  }
