...
const server = http.createServer(async (req, res) => {
  ...
  // --- Live Testnet wallet balance (Xaman-linked lobby) ---
  if (req.method === "GET" && url.pathname === "/xrp/balance") {
    const address = (url.searchParams.get("address") || "").trim();
    if (!xaman.isClassicAddress(address)) {
      sendJson(res, 400, { ok: false, reason: "That is not a classic XRP address." });
      return;
    }
    try {
      const out = await xaman.getAccountXrp(address);
      sendJson(res, 200, out);
    } catch (e) {
      sendJson(res, 502, { ok: false, reason: e.message || "Could not read Testnet balance" });
    }
    return;
  }
  ...
  if (req.method === "POST" && url.pathname === "/airdrop/whitelist") {
    try {
      ...
      const level = Number(body && body.level) || 0;
      const rank = String((body && body.rank) || "").slice(0, 40);
      const exp = Number(body && body.exp) || 0;
      if (!xaman.isClassicAddress(address)) {
        sendJson(res, 400, { ok: false, reason: "That is not a classic XRP address." });
        return;
      }
      if (level < 3) {
        sendJson(res, 400, { ok: false, reason: "Level 3+ required" });
        return;
      }
      const entry = {
        address,
        ...
        exp,
        at: Date.now(),
        reason: String((body && body.reason) || "level3").slice(0, 32),
      };
      const out = airdropWhitelist.upsert(entry);
      const list = airdropWhitelist.getList();
      sendJson(res, 200, {
        ok: true,
        replay: !!out.replay,
        entry: out.entry,
      ...
      sendJson(res, 500, { ok: false, reason: e.message || "Whitelist failed" });
    }
    return;
  }
  if (req.method === "GET" && url.pathname === "/airdrop/whitelist") {
    const list = airdropWhitelist.getList();
    sendJson(res, 200, {
      ok: true,
      count: list.length,
      entries: list.slice(-500),
      exportCsv: "/airdrop/whitelist.csv",
      exportJson: "/airdrop/whitelist.json",
    });
    return;
  }
  if (req.method === "GET" && url.pathname === "/airdrop/whitelist.csv") {
    const list = airdropWhitelist.getList();
    const csv = airdropWhitelist.toCsv(list);
    cors(res);
    res.writeHead(200, {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": 'attachment; filename="apex-airdrop-whitelist.csv"',
      "Cache-Control": "no-store",
    });
    res.end(csv);
    return;
  }
  if (req.method === "GET" && url.pathname === "/airdrop/whitelist.json") {
    const list = airdropWhitelist.getList();
    cors(res);
    res.writeHead(200, {
      "Content-Type": "application/json; charset=utf-8",
      "Content-Disposition": 'attachment; filename="apex-airdrop-whitelist.json"',
      "Cache-Control": "no-store",
    });
    res.end(JSON.stringify({ ok: true, count: list.length, exportedAt: Date.now(), entries: list }, null, 2));
    return;
  }
